...
import { Appointment, Customer, Service, Staff, AppointmentTableRow } from './types';
...
export class AppointmentService {
  ...
  static async getAppointmentsWithDetails(): Promise<AppointmentTableRow[]> {
    try {
      ...
      const [appointmentsSnapshot, customersSnapshot, servicesSnapshot, staffSnapshot] = 
        await Promise.all([
      ...
      const appointments: AppointmentTableRow[] = appointmentsSnapshot.docs.map((doc, index) => {
        ...
        const totalPrice = appointmentData.totalPrice || 
          services.reduce((sum, service) => sum + service.price, 0);
        
        const totalTimeframe = appointmentData.totalTimeframe || 
  ...
  static async getAppointmentsByCustomer(customerId: string): Promise<AppointmentTableRow[]> {
    try {
      ...
      return appointmentsSnapshot.docs.map((doc, index) => {
        ...
        const totalTimeframe = appointmentData.totalTimeframe || 
          appointmentServices.reduce((sum, service) => sum + service.duration, 0);
  ...
  static async getAppointmentsByStatus(status: Appointment['status']): Promise<AppointmentTableRow[]> {
    try {
      const appointmentsQuery = query(
        collection(db, 'appointments'),
        where('status', '==', status),
  ...
  private static async getCustomerById(customerId: string): Promise<Customer | null> {
    try {
  ...
  private static async getServicesByIds(serviceIds: string[]): Promise<Service[]> {
